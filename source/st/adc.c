...
struct adc_device {
	ADC_TypeDef *base;
	uint16_t data[MAX_ADC_CHAN];
	int (*adc_handler[MAX_ADC_CHAN])(void *private_data, uint32_t adc_value);
	void *private_data[MAX_ADC_CHAN];
	uint8_t chan;
	uint8_t wp;
};
...
static int dma2_steam0_irq_handler(void *private_data)
{
	...
	if (DMA_GetFlagStatus(DMA2_Stream0, DMA_FLAG_TCIF0)) {
		for (i = 0; i < adc_dev->chan; i++) {
			if (!adc_dev->adc_handler[i])
				continue;

			adc_dev->adc_handler[i](adc_dev->private_data[i], adc_dev->data[i]);
		}
	...
	int (*adc_handler)(void *private_data, uint32_t adc_value), void *private_data)
{
	...
	if (adc >= MAX_ADC_NUM || chan >= MAX_ADC_CHAN)
		return -EINVAL;

 	adc_dev = &adc_devs[adc];
	configASSERT(!adc_dev->adc_handler[chan]);
	adc_dev->adc_handler[chan] = adc_handler;
	adc_dev->private_data[chan] = private_data;
	return 0;
}
...
void adc_unregister_handler(uint8_t adc, uint8_t chan)
{
	...
	if (adc >= MAX_ADC_NUM || chan >= MAX_ADC_CHAN)
 	...
 	adc_dev = &adc_devs[adc];
	configASSERT(adc_dev->adc_handler[chan]);
	adc_dev->adc_handler[chan] = NULL;
	adc_dev->private_data[chan] = NULL;
}
