...
static int spi_wrapper_write_once(uint8_t spi_id, uint8_t data)
{
...
static uint8_t spi_wrapper_read_once(uint8_t spi_id)
{
	while (SPI_I2S_GetITStatus(SPIx, SPI_I2S_IT_RXNE) == RESET);
	return SPI_I2S_ReceiveData(SPIx);
}

int spi_wrapper_write(uint8_t spi_id, uint8_t reg, uint8_t data)
{
	SPI_Cmd(SPIx, ENABLE);
	spi_wrapper_write_once(spi_id, reg);
	spi_wrapper_write_once(spi_id, data);
	SPI_Cmd(SPIx, DISABLE);
	return 0;
}

int spi_wrapper_read(uint8_t spi_id, uint8_t reg, uint8_t **data)
{
	SPI_Cmd(SPIx, ENABLE);
	spi_wrapper_write_once(spi_id, reg);
	spi_dev->rx_buf[0] = spi_wrapper_read_once(spi_id);
	SPI_Cmd(SPIx, DISABLE);
	*data = spi_dev.rx_buf;
	return 0;
}
...
int spi_wrapper_multi_read(uint8_t spi_id, uint8_t reg, uint8_t **data, uint8_t len)
{
	...
	spi_wrapper_write_once(spi_id, reg);
	for (i = 0; i < len; i++)
		spi_dev.rx_buf[i] = spi_wrapper_read_once(spi_id);
	SPI_Cmd(SPIx, DISABLE);
	*data = spi_dev.rx_buf;
	return 0;
}
